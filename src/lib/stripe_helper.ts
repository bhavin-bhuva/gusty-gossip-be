...
class StripeHelper {
  ...
  public async createCheckoutSession(
    ...
    try {
      ...
      if (!utility.isEmpty(params.searchKeyword)) {
        SUCCESS_URL = `search/${params.searchKeyword}/${params.prodId}`;
        CANCEL_URL = `search/${params.searchKeyword}/${params.prodId}/failure`;
      }
      ...
      const sessionParams: Stripe.Checkout.SessionCreateParams = {
        ...
        success_url: `${process.env.BASE_URL}${SUCCESS_URL}?session_id={CHECKOUT_SESSION_ID}&${
          params.isCopy && params.isCopy == 'true' ? 'isCopy=true' : 'isCopy=false'
        }`,
        cancel_url: `${process.env.BASE_URL}${CANCEL_URL}?status=failed&session_id={CHECKOUT_SESSION_ID}`,
  ...
  public async getSubscriptionList(subId: string): Promise<any> {
    try {
      const subscriptions = await this.stripe.subscriptions.retrieve(subId);
  ...
  public async getInvoice(invoiceId: string): Promise<Stripe.Invoice> {
    try {
  ...
  public async getPaymentIntent(intentId: string): Promise<Stripe.PaymentIntent> {
    try {
